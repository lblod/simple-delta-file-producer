import { LOG_DELTA_REWRITE, TYPE_CONFIG } from './config.js';
 ...
 *
 * For now this only includes ignoring a fixed set of types defined in the variable TYPE_CONFIG.ignored
 */
...
async function isTripleCleared(triple) {
  let cleared = true;
  for (let ignored of TYPE_CONFIG.ignored) {
    cleared = !(await hasType(triple, ignored.type));
  }
  return cleared;
}
